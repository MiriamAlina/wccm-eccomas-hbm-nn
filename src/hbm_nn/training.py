...
def load_and_scale_data(data_file: str):
    """
    ...
    """
    train = np.load(f'data/train_{data_file}.npz')
    val = np.load(f'data/val_{data_file}.npz')
    test = np.load(f'data/test_{data_file}.npz')
...
def fit(
    ...
    early_stopper = (
        EarlyStopping(config.early_stopping_patience,
                      config.early_stopping_min_delta)
        if config.early_stopping_patience is not None else None
    )

    if config.verbose == 1:
        print(f'Training: hidden={config.hidden_sizes}, '
    ...
    for epoch in range(config.n_epochs):
        ...
        for i in range(0, len(X_tr), config.batch_size):
            ...
            n_batches += 1
        # Single .item() call per epoch
        # the only CPU sync in the training pass
        train_loss = (epoch_loss / n_batches).item()
        train_losses.append(train_loss)

        # --- validation pass ---
        # model.eval() disables dropout and switches batchnorm to running
        # stats, giving a deterministic output that reflects inference-time
        # behaviour.
        ...
        if val_loss < best_val_loss:
            ...
            # clone keeps them independent of the live model
            best_state = {k: v.clone() for k, v in model.state_dict().items()}
        ...
        if early_stopper is not None and early_stopper.step(val_loss):
            if config.verbose >= 1:
                print(f'  Early stopping triggered after {epoch + 1} epochs '
                      f'({config.early_stopping_patience} epochs without '
                      'improvement).')
        ...
        # If the callback signals that this trial is unpromising
        # (returns True), we exit immediately and let the caller raise
        # TrialPruned.
    ...
    model.load_state_dict(best_state)
    if config.verbose >= 1:
        print(f'Done. Best val loss: {best_val_loss:.3e} '
              f'(epoch {val_losses.index(best_val_loss)})')
    return {
        'train_losses':  train_losses,
        'val_losses':    val_losses,
        'best_val_loss': best_val_loss,
...
def save_artifacts(
    ...
    torch.save(model, f'models/mlp_{model_id}.pt')
    joblib.dump(history, f'models/losses_{model_id}.joblib')
    joblib.dump(scaler,  f'models/scaling_params_{model_id}.joblib')
...
def load_artifacts(model_id: str) -> tuple[nn.Module, dict, dict]:
    ...
    """
    model = torch.load(f'models/mlp_{model_id}.pt', weights_only=False)
    scaler = joblib.load(f'models/scaling_params_{model_id}.joblib')
    history = joblib.load(f'models/losses_{model_id}.joblib')
    model.eval()
    return model, scaler, history


def display_model_info(model: nn.Module, scaler: dict, history: dict,
                       model_id: str = '') -> None:
    ...
    - Training history: epochs run, best validation loss, early-stopping
        status.
    ...
    header = f'  Model: mlp_{model_id}' if model_id else '  Model'
    print(f'\n{sep}\n{header}\n{sep}')
    ...
    print(f'  Best val loss    : {history["best_val_loss"]:.4e}  '
          f'(epoch {best_epoch})')
    print(f'  Final train loss : {train_losses[-1]:.4e}')
    print(f'  Final val loss   : {val_losses[-1]:.4e}')
    print('  Early stopped    : '
          f'{"Yes" if history["stopped_early"] else "No"}')
